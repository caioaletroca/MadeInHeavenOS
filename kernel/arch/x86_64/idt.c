...
void PIC_remap(int offset1, int offset2)
{
	...
	outb(PIC1_DATA, 0xFB);   // restore saved masks.
	outb(PIC2_DATA, 0xFF);
}
 ...
 * @param flags 
 */
static void idt_entry_init(unsigned int i, uint64_t offset, uint16_t selector, uint8_t flags) {
...
void idt_init(void) {
    ...
    __asm__ __volatile__("lidt (%0)": : "r"(idt_addr));
